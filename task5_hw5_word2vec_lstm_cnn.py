...
class TextClassifier(nn.Module):
    def __init__(self) -> None:
        ...
        self.conv1d = nn.Conv1d(in_channels=w2v_embedding.shape[1], out_channels=num_filters, kernel_size=kernel_size, padding=kernel_size//2)
        self.lstm = nn.LSTM(input_size=num_filters, hidden_size=hidden_size, dropout=0.2, num_layers=2, bidirectional=True, batch_first=True)
        self.fc = nn.Linear(hidden_size * 2, num_classes)

    def forward(self, x):
        x = x.permute(0, 2, 1)  # 调整为 [batch_size, embed_size, seq_len] 以匹配 CNN 输入格式

        # 卷积层提取局部特征
        conv_out = self.conv1d(x)  # [batch_size, num_filters, seq_len]
        conv_out = torch.relu(conv_out)

        conv_out = conv_out.permute(0, 2, 1)  # 调整为 [batch_size, seq_len, num_filters] 以匹配 LSTM 输入格式

        # LSTM 层捕捉序列依赖
        lstm_out, _ = self.lstm(conv_out)  # [batch_size, seq_len, hidden_size*2]
        lstm_out = torch.sum(lstm_out, dim=1)  # 句子级表示
